...
# try to change: timesteps, bentodelratio, and sb
timeSteps=10000
initialPopsize=2000
mud=2
chromosomesize=10
...
calcfixation=0
#max rate of reaction (Vmax) and Michaelis-Menten constant (Km) for recessivity runs
maxRateOfReaction=1
michaelisConstant=0.09
#0 for no recessivity, 1 for recessivity
recessivityRunFlag=1
...
./mutationload $timeSteps $initialPopsize $mud $chromosomesize $numberofchromosomes $bentodelratio $sb $bendist $typeofrun $slope $seed $K $fitnesstype $r $i_init $s $tskitstatus $modularepis $elementsperl $snapshot $file1 $SdtoSbratio $deldist $rawdatafilesize $redinmaxpopsize $calcfixation $maxRateOfReaction $michaelisConstant $recessivityRunFlag
