...
'''
File: tskitAnalysis_growth.py
Author: Micaila Marcelle
Purpose: Analyzes the tskit output from a given simulation, uses this to 
construct the SFS, then feeds this SFS into dadi and tries to fit a growth
model to it. Used to generate Fig. 2 and Fig. 4 within paper. 
'''
...
for i in range(0, len(X)):
    ...
    inverses_coal_ne.append((2 * (2 * coal_ne) * rate) / X[i])
...
for i in range(20):
    ...
    fig.savefig("dadi_model_fit_growth_ss2000_" + str(i) + ".png")
    ...
    for j in range(0, len(X)):
        dadi_inverses_n.append(((4 * N * rate) / X[j]))
        dadi_inverses_ne.append(((4 * (2 * coal_ne) * rate) / X[j]))
        
    norm_val_dadi = dadi_inverses_n[0]
    for j in range(0, len(X)):
        dadi_inverses_n[j] /= norm_val_dadi
        dadi_inverses_ne[j] /= norm_val_dadi
    ...
    generated_sfs = dadi.Inference.optimally_scaled_sfs(generated_sfs, reduced_sfs)
    ...
    for j in range(len(reduced_list_sfs)):
        reduced_list_sfs[j] /= ((norm_val / 2) * L)
        model_list_sfs[j] /= ((norm_val / 2) * L)
    ...
    # Sets appropriate axis labels for the plot
    plt.xlabel("Allele count / 2N", fontweight = "bold", fontsize = 20)
    plt.ylabel("Normalized number of sites", fontweight = "bold", fontsize = 20)
    ...
    fig.set_figheight(8.5)
    fig.set_figwidth(13)
    plt.savefig("comp_observed_model_2000_growth_scaled_" + str(i) + ".png")
